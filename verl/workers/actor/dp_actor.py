...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _prepare_multimodal_tensor_for_cat(self, key: str, value: torch.Tensor) -> torch.Tensor:
        """Normalize one sample's multimodal tensor before merging a micro-batch.

        Qwen-VL processors can return per-example visual tensors with a leading
        singleton batch dimension, e.g. pixel_values shaped [1, visual_tokens, dim].
        Different images naturally have different visual_tokens, so concatenating
        those tensors directly along dim=0 fails because dim=1 differs.  For those
        ragged visual tensors, remove the fake batch dimension first so torch.cat
        joins the visual-token dimension instead.
        """
        ragged_visual_keys = {
            "pixel_values",
            "pixel_values_videos",
        ...
        if key in ragged_visual_keys and isinstance(value, torch.Tensor):
            if value.dim() >= 3 and value.shape[0] == 1:
                value = value.squeeze(0)

        return value

    def _merge_multi_modal_inputs(self, micro_batch) -> dict:
        """Merge per-sample multimodal processor outputs for a micro-batch.

        This is more tolerant than the original implementation for mixed
        text/image batches.  Text-only rows may have an empty multimodal dict,
        while image rows may have Qwen-VL visual tensors with variable visual
        token lengths.  We skip empty dicts and concatenate only keys that are
        actually present.
        """
        if "multi_modal_inputs" not in micro_batch:
            return {}

        mm_inputs_list = micro_batch["multi_modal_inputs"]
        if mm_inputs_list is None:
            return {}

        valid_mm_inputs = [
            inputs for inputs in mm_inputs_list
            if isinstance(inputs, dict) and len(inputs) > 0
        ]
        if len(valid_mm_inputs) == 0:
            return {}

        merged = {}
        all_keys = set().union(*(inputs.keys() for inputs in valid_mm_inputs))

        for key in all_keys:
            # Qwen3-VL processors may emit mm_token_type_ids with one length per
            # prompt, e.g. [1, 527] and [1, 338]. In this verl path, input_ids
            # and position_ids are already padded/unpadded by the actor, and the
            # image locations are carried by the tokenized prompt plus
            # image_grid_thw/pixel_values. Passing ragged per-prompt
            # mm_token_type_ids causes micro-batch concatenation failures, and it
            # is not needed for the log-prob forward here.
            if key == "mm_token_type_ids":
                continue

            values = [
                inputs[key] for inputs in valid_mm_inputs
                if key in inputs and inputs[key] is not None
            ]
            if len(values) == 0:
                continue

            # Special handling for MiniCPM-o model: pixel_values, image_bound,
            # and tgt_sizes need different concatenation strategies compared to
            # other multimodal inputs.
            if (key == "pixel_values" and isinstance(values[0], list)) or key in {"image_bound", "tgt_sizes"}:
                merged[key] = values
                continue

            if all(isinstance(value, torch.Tensor) for value in values):
                values = [self._prepare_multimodal_tensor_for_cat(key, value) for value in values]
                try:
                    merged[key] = torch.cat(values, dim=0)
                except RuntimeError as exc:
                    shapes = [tuple(value.shape) for value in values]
                    raise RuntimeError(
                        f"Failed to concatenate multimodal input key={key!r}. "
                        f"Shapes after normalization: {shapes}"
                    ) from exc
            else:
                # Preserve non-tensor multimodal inputs as a list.  This mirrors
                # the original MiniCPM-o behavior and avoids assuming every
                # processor output is a tensor.
                merged[key] = values
    ...
    def _forward_micro_batch(self, micro_batch, temperature, calculate_entropy=False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        multi_modal_inputs = self._merge_multi_modal_inputs(micro_batch)
